...
Berikan lingkungan Anda sendiri Microsoft Dataverse untuk berbagi data (profil dan wawasan) dengan aplikasi bisnis berdasarkan Dataverse, seperti Dynamics 365 Marketing atau aplikasi berbasis model di Indonesia Power Apps.
...
Mengaktifkan berbagi data dengan Microsoft Dataverse memilih kotak centang berbagi data akan memicu penyegaran penuh satu kali sumber data Anda dan semua proses lainnya.
...
> 1. Tidak ada lingkungan Customer Insights yang sudah terkait dengan lingkungan tersebut Dataverse. Pelajari cara [menghapus koneksi yang ada ke Dataverse lingkungan](#remove-an-existing-connection-to-a-dataverse-environment).
...
### <a name="enable-data-sharing-with-dataverse-from-your-own-azure-data-lake-storage-preview"></a>Aktifkan berbagi data dari Dataverse Anda sendiri Azure Data Lake Storage (Pratinjau)

Jika lingkungan Anda dikonfigurasi untuk menggunakan milik Anda sendiri Azure Data Lake Storage untuk menyimpan data Wawasan Pelanggan, memungkinkan berbagi data dengan Microsoft Dataverse perlu beberapa konfigurasi tambahan.
...
1. Instal powershell untuk graph [Azure Active Directory versi](/powershell/azure/active-directory/install-adv2) terbaru.
...
1. Unduh dua skrip PowerShell yang perlu Anda jalankan dari repo [GitHub insinyur](https://github.com/trin-msft/byol) kami.
    1. `CreateSecurityGroups.ps1`
       ...
       - Skrip PowerShell ini membuat dua grup keamanan pada langganan Azure Anda. Satu untuk kelompok Pembaca dan satu lagi untuk grup kontributor dan akan menjadikan Microsoft Dataverse layanan sebagai pemilik untuk kedua kelompok keamanan ini.
       ...
       - Simpan kedua nilai ID grup keamanan yang dihasilkan oleh skrip ini karena kami akan menggunakannya dalam `ByolSetup.ps1` skrip.
       
        > [!NOTE]
        > Pembuatan grup keamanan dapat dinonaktifkan di penyewa Anda. Dalam hal ini, pengaturan manual akan diperlukan dan admin Anda Azure AD harus [mengaktifkan pembuatan](/azure/active-directory/enterprise-users/groups-self-service-management) grup keamanan.
    ...
    2. `ByolSetup.ps1`
        ...
        - Skrip PowerShell ini menambahkan kontrol akses berbasis tole yang diperlukan (RBAC) untuk Microsoft Dataverse layanan dan aplikasi bisnis berbasis apa pun Dataverse. Ini juga memperbarui Daftar Kontrol Akses (ACL) pada kontainer CustomerInsights untuk grup keamanan yang `CreateSecurityGroups.ps1` dibuat dengan skrip. Grup kontributor akan memiliki *izin rwx* dan grup Pembaca hanya akan memiliki *izin r-x*.
        - Jalankan skrip PowerShell ini di Windows PowerShell dengan menyediakan ID langganan Azure yang berisi Azure Data Lake Storage nama akun penyimpanan, nama grup sumber daya, dan nilai id grup Pembaca dan kontributor keamanan. Buka skrip PowerShell di editor untuk meninjau informasi tambahan dan logika yang diterapkan.
...
2. Masukkan string output yang disalin dari atas ke **dalam bidang pengenal** Izin dari langkah konfigurasi lingkungan untuk Microsoft Dataverse.
...
- Jika Anda mengaktifkan berbagi data dengan Dataverse, Anda tidak akan dapat melihat laporan PowerBI Embedded opsional di lingkungan Wawasan Pelanggan Anda.
...
Saat terhubung ke Dataverse lingkungan, pesan **kesalahan Organisasi CDS ini sudah dilampirkan ke instans** Customer Insights lain berarti Dataverse lingkungan sudah digunakan di lingkungan Wawasan Pelanggan. Anda dapat menghapus koneksi yang ada sebagai administrator global di Dataverse lingkungan. Diperlukan beberapa jam untuk mengisi perubahan.

1. Tuju [Power Apps](https://make.powerapps.com).
1. Pilih lingkungan dari pemetik lingkungan.
1. **Buka Solusi**
...
- Rahasia koneksi yang digunakan untuk ekspor dan pengayaan. Anda harus mengotorisasi ulang koneksi dan kemudian mengaktifkan kembali pengayaan dan ekspor. 
